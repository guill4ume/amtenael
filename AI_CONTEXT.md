...
- **Logging** : Le serveur SPB actuel utilise encore `log4net` (via `ILog log = ...`). Le USER a confirmé qu'une migration vers NLog n'est pas souhaitée pour le moment.
...
- **Population de Bots** : Configurée dans `MimicBattlegrounds.Initialize()`. Actuellement fixée à **15 bots total** (5 par royaume) pour garantir la stabilité et la fluidité.